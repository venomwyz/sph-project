...
public class WxConsumer {
    ...
    public void orderPayMessage(Channel channel, Message message){
        ...
        try {
            //修改订单的状态
            orderService.updatePayStatus(payResultString);
            //确认消息
            ...
            try {
                ...
                if(messageProperties.getRedelivered()){
                    //两次都失败
                    log.error("连续两次修改订单的支付结果都失败,支付报文为:" + payResultString);
                    //拒绝消息,并丢弃
                ...
                log.error("拒绝消息发生异常,修改订单的支付结果可能失败了,需要确认,支付报文为:" + payResultString);
            }
        }
    }
